...
class bdd100k_dd(imdb):
    def __init__(self, image_set, use_diff=False):
        imdb.__init__(self, "bdd100k" + "_dd_" + image_set)
    ...
    def gt_roidb(self):
        ...
        """
        cache_file = os.path.join("/data2/phat/DATASET/cache/bdd100k_cache/", self.name + '_gt_roidb.pkl')
        ...
        with open(os.path.join(self._devkit_path, 'labels', 'bdd100k_labels_images_%s.json' % self.mode), 'rb') as f:
            annots = json.load(f)
            for ann in annots:
                if ann['attributes']["timeofday"] == "dawn/dusk":
    ...
    def _do_python_eval(self, output_dir='output'):
        ...
        imagesetfile = os.path.join(
            self._devkit_path,
            'ImageSets',
            self._image_set + '_dawn_dusk.txt')
