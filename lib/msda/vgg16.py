...
class vgg16(_fasterRCNN):
    ...
    def _init_modules(self):
        ...
        # print(vgg.features)
        self.RCNN_base1_sub1 = copy.deepcopy(nn.Sequential(*list(vgg.features._modules.values())[:14]))
        self.RCNN_base1_sub2 = copy.deepcopy(nn.Sequential(*list(vgg.features._modules.values())[:14]))
        self.RCNN_base1_sub_ema = copy.deepcopy(nn.Sequential(*list(vgg.features._modules.values())[:14]))
        ...
        self.netD1 = netD(context=self.gc)
        self.netD2 = netD(context=self.gc)
        ...
        for layer in range(10):
            for p in self.RCNN_base1_sub1[layer].parameters():
                p.requires_grad = False
            for p in self.RCNN_base1_sub2[layer].parameters():
                p.requires_grad = False
            for p in self.RCNN_base1_sub_ema[layer].parameters():
                p.requires_grad = False
    ...
    def _head_to_tail1(self, pool5):

        pool5_flat = pool5.view(pool5.size(0), -1)
        fc7 = self.RCNN_top1(pool5_flat)

        return fc7

    def _head_to_tail2(self, pool5):

        pool5_flat = pool5.view(pool5.size(0), -1)
        fc7 = self.RCNN_top2(pool5_flat)

        return fc7

    def _head_to_tail_ema(self, pool5):

        pool5_flat = pool5.view(pool5.size(0), -1)
        fc7 = self.RCNN_top_ema(pool5_flat)
