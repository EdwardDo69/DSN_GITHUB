...
class _fasterRCNN(nn.Module):
    ...
    def forward(
            self, im_data, im_info,  gt_boxes, num_boxes, target=False,subnet=False, eta=1.0, have_base_feat2=False,
            subnet1_base_feat2 = None, subnet2_base_feat2 = None,
    ):
        ...
        if subnet == "subnet1":
            base_feat1 = self.RCNN_base1_sub1(im_data)
        elif subnet == "subnet2":
            base_feat1 = self.RCNN_base1_sub2(im_data)
        elif subnet == "ema":
            base_feat1 = self.RCNN_base1_sub_ema(im_data)
        ...
        if subnet == "subnet1":
            ...
            if self.training:
                ...
                rpn_bbox_pred1 = self.RCNN_rpn1(
                    base_feat_1, im_info, gt_boxes, num_boxes, ema=True
                )
                rpn_bbox_pred2 = self.RCNN_rpn2(
                    base_feat_2, im_info, gt_boxes, num_boxes, ema=True
                )
                rpn_bbox_pred_ema = self.RCNN_rpn_ema(
                    base_feat, im_info, gt_boxes, num_boxes, ema=True
                )
    ...
    def step(self, lmb1, lmb2):
        ...
        for ema_param, param1, param2 in zip(self.RCNN_base1_sub_ema.parameters(), self.RCNN_base1_sub1.parameters(),
                                             self.RCNN_base1_sub2.parameters()):
