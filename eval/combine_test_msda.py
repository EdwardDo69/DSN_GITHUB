...
if __name__ == "__main__":
    ...
    all_boxes_sub1_sub2 = [[[] for _ in xrange(num_images)] for _ in xrange(imdb.num_classes)]
    all_boxes_sub1 = [[[] for _ in xrange(num_images)] for _ in xrange(imdb.num_classes)]
    all_boxes_sub2 = [[[] for _ in xrange(num_images)] for _ in xrange(imdb.num_classes)]
    all_boxes_ema = [[[] for _ in xrange(num_images)] for _ in xrange(imdb.num_classes)]
    ...
    for i in range(num_images):
        ...
        for j in xrange(1, imdb.num_classes):
            ...
            if inds.numel() > 0:
                ...
                all_boxes_sub1_sub2[j][i] = cls_dets.cpu().numpy()
            else:
                all_boxes_sub1_sub2[j][i] = empty_array

        # Limit to max_per_image detections *over all classes*
        if max_per_image > 0:
            image_scores = np.hstack(
                [all_boxes_sub1_sub2[j][i][:, -1] for j in xrange(1, imdb.num_classes)]
            )
            if len(image_scores) > max_per_image:
                image_thresh = np.sort(image_scores)[-max_per_image]
                for j in xrange(1, imdb.num_classes):
                    keep = np.where(all_boxes_sub1_sub2[j][i][:, -1] >= image_thresh)[0]
                    all_boxes_sub1_sub2[j][i] = all_boxes_sub1_sub2[j][i][keep, :]
        ...
        for j in xrange(1, imdb.num_classes):
            ...
            if inds.numel() > 0:
                ...
                all_boxes_sub1[j][i] = cls_dets.cpu().numpy()
            else:
                all_boxes_sub1[j][i] = empty_array

        # Limit to max_per_image detections *over all classes*
        if max_per_image > 0:
            image_scores = np.hstack(
                [all_boxes_sub1[j][i][:, -1] for j in xrange(1, imdb.num_classes)]
            )
            if len(image_scores) > max_per_image:
                image_thresh = np.sort(image_scores)[-max_per_image]
                for j in xrange(1, imdb.num_classes):
                    keep = np.where(all_boxes_sub1[j][i][:, -1] >= image_thresh)[0]
                    all_boxes_sub1[j][i] = all_boxes_sub1[j][i][keep, :]
        ...
        for j in xrange(1, imdb.num_classes):
            ...
            if inds.numel() > 0:
                ...
                all_boxes_sub2[j][i] = cls_dets.cpu().numpy()
            else:
                all_boxes_sub2[j][i] = empty_array

        # Limit to max_per_image detections *over all classes*
        if max_per_image > 0:
            image_scores = np.hstack(
                [all_boxes_sub2[j][i][:, -1] for j in xrange(1, imdb.num_classes)]
            )
            if len(image_scores) > max_per_image:
                image_thresh = np.sort(image_scores)[-max_per_image]
                for j in xrange(1, imdb.num_classes):
                    keep = np.where(all_boxes_sub2[j][i][:, -1] >= image_thresh)[0]
                    all_boxes_sub2[j][i] = all_boxes_sub2[j][i][keep, :]
        ...
        for j in xrange(1, imdb.num_classes):
            ...
            if inds.numel() > 0:
                ...
                all_boxes_ema[j][i] = cls_dets.cpu().numpy()
            else:
                all_boxes_ema[j][i] = empty_array

        # Limit to max_per_image detections *over all classes*
        if max_per_image > 0:
            image_scores = np.hstack(
                [all_boxes_ema[j][i][:, -1] for j in xrange(1, imdb.num_classes)]
            )
            if len(image_scores) > max_per_image:
                image_thresh = np.sort(image_scores)[-max_per_image]
                for j in xrange(1, imdb.num_classes):
                    keep = np.where(all_boxes_ema[j][i][:, -1] >= image_thresh)[0]
                    all_boxes_ema[j][i] = all_boxes_ema[j][i][keep, :]
    ...
    print("----------------sub1, sub2--------------------------------------")
    imdb.evaluate_detections(all_boxes_sub1_sub2, args.output_dir)
    print("-----------------sub1------------------------------------------------")
    imdb.evaluate_detections(all_boxes_sub1, args.output_dir)
    print("-----------------sub2------------------------------------------------")
    imdb.evaluate_detections(all_boxes_sub2, args.output_dir)
    print("-----------------ema------------------------------------------------")
    imdb.evaluate_detections(all_boxes_ema, args.output_dir)
    print("---------------------------------------------------------------------")
